...
- [x] **Remove `moduleSystem` difference from import path logic** — `makeRelativeImportPath` adds a `.js` extension for ESM but not for CJS. The reason for this asymmetry is not documented and is surprising (CJS modules also use `.js`). Investigate if this is actually correct, document why in the code if it is, or fix the inconsistency. **(Addressed in v2 by adding a new `importExtension: 'js' | 'ts' | 'none'` option that directly describes what is emitted at the import-specifier tail. The legacy `moduleSystem` option was kept as a deprecated alias for one minor cycle — `'esm' → 'js'`, `'cjs' → 'none'` — then removed in v3. The new `'ts'` value supports native TypeScript runtimes like Bun/Deno that the boolean ESM/CJS axis could not express.)**
