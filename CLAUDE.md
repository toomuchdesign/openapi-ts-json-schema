...
- **`src/utils/makeTsJsonSchema/`** — Handles the three `refHandling` strategies (`import`, `inline`, `keep`) and emits TypeScript source. `emitTsSchema.ts` is a single recursive emitter that walks the dereferenced schema, resolves inlined-ref markers (`REFERENCED_SCHEMA_ID_SYMBOL`) into identifier references / `$ref` literals / inlined values, renders `LEADING_COMMENT_SYMBOL` annotations as JS comments, and short-circuits cycles to `{}`. `index.ts` wraps the emitter output with the import block, optional `@deprecated` JSDoc, and the `as const` suffix (skipped when the file is just a re-export alias).
