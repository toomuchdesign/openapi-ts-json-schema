...
targets – Specifies which parts of the OpenAPI document to convert.

single – paths pointing to individual definitions to generate.

collections – paths pointing to objects/records of definitions, where each entry will be generated.
...
| **targets** _(required)_         | `{`</br>` collections?: string[];`</br>` single?: string[];`</br>`}` | Specifies which parts of the OpenAPI document to convert _(dot notation)_.<br/><br/>`collections`: paths pointing to objects/records of definitions, where each entry will be generated (eg: `["components.schemas"]`).<br/>`single`: paths pointing to individual definitions to generate (eg: `["paths./users/{id}"]`). | -          |
